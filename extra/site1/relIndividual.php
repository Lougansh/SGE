...
if (isset($_POST['enviar'])&& $_POST['cgm'] != '') { 
$cgm = $_POST['cgm'] ;
	$sql = "select * from tb_aluno_projeto where cgm = '$cgm'";
	$query = mysql_query($sql) or die("SQL:" . $sql . " - ERRO:" . mysql_error());
	while ($linha = mysql_fetch_array($query)) {
		$ID = $linha['cgm'];
		$aluno = '<br><hr  width="70%"><h3 align="center">'.$linha['nomeAluno'].'</h3><hr  width="50%">';
		$obs = $linha['obs'];
		$foto = '<img border="0" src="../images/CFH/'.$ID.'.JPG" height="300">';
		$sexo = $linha['sexo'];
		...
		if($linha['nomePai'] != ''){$pai = $linha['nomePai'].' e ';} 
		if($linha['nomeMae'] != ''){$mae = $linha['nomeMae'];}
		if ($signo == 'aries'){
			if ($sexo == 'F'){
			$retSigno = '
		...
		$descricao = '
		'.$filho.' de '.$pai.$mae.'.<br><br>
		Nasceu no dia '.$dia.'/'.$mes.'/'.$ano.'. 
		Hoje está com '.$anos.'. '.$nativo.' do signo de '.$signo.'. '.$retSigno.' '.$obs.'';
	}
}
echo'
<head><meta http-equiv="Content-Language" content="pt-br"></head>
<h2 align="center">Relatório Individual do Curso de Robótica</h2>
<form method="POST" action="?id=18" onchange="form.submit()">
	<p align="center">&nbsp;Digite o código do aluno
	<input type="text" name="cgm" size="20">
	<input type="submit" value="Enviar" name="enviar">
	<input type="reset" value="Limpar" name="limpar"></p>
	</form>
<div align="center">
	<table border="0" style="border-collapse: collapse" width="70%" bordercolorlight="#808080" bordercolordark="#000000">
		<tr>
			<td width="40%">
			'.$foto.'
			<td valign="top">
