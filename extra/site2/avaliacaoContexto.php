...
$sql = "select aluno.*, turma.Turno FROM tb_aluno aluno, tb_turma turma WHERE aluno.turma = turma.turma and aluno.ano = turma.ano and aluno.Situacao = 'A' and aluno.avaliacaoContexto = 'S'order by aluno.ano desc, turma.turma desc";	
...
$encaminhamentos = $linha['encaminhamento'];
$avancos = $linha['avanco'];
if ($linha['avaliacaoContexto']	== 'S') {$avaliacaoContexto='<input type="radio" value="S" checked name="chkAvaliacaoContexto">SIM<input type="radio" value="N" name="chkAvaliacaoContexto">NAO';} else {$avaliacaoContexto=  '<input type="radio" value="S" name="chkAvaliacaoContexto">SIM<input type="radio" value="N" checked name="chkAvaliacaoContexto">NAO';}

echo'
<div align="center"><table border="0" width="730">
<tr><td rowspan="10" align="center"><p><hr width="10%"><hr width="30%"><hr width="50%"><img src="../images/alunos/'.$linha['ID'].'.JPG" width="231" height="328"><hr width="50%"><hr width="30%"><hr width="10%"><select size="1" name="dropCodigo"><option selected value="'.$linha['ID'].'">'.$linha['ID'].'</option></select><br><input type="submit" value="Imprimir" name="btnImprimir"></td></tr>
...
<tr><td  align="right">Avaliação em Contexto:	</td><td><font color="green">'.$avaliacaoContexto.' </font></td></tr>
...
Desempenho:<br><textarea rows="3" name="tedDesempenho" cols="70">' . $desempenho .'</textarea><br>
Encaminhamentos:<br><textarea rows="3" name="tedEncaminhamentos" cols="70">' . $encaminhamentos .'</textarea><br>
Avanços:<br><textarea rows="3" name="tedAvancos" cols="70">' . $avancos .'</textarea></td></tr></table><hr width="50%"></div></form>
';
}
if (isset($_POST['btnImprimir'])){
	$_SESSION["ID"] = $_POST['dropCodigo'];
	echo "<script>window.setTimeout(\"location.href='fichaAvaliacaoContexto.php'\",3)</script>";
}
...
$paiUP 		= $_POST['dropPai'];
$maeUP 		= $_POST['dropMae'];
$desempenhoUP	= $_POST['tedDesempenho'];
$encaminhamentosUP	= $_POST['tedEncaminhamentos'];
$avancosUP	= $_POST['tedAvancos'];
...
avaliacaoContexto = '$avaliacaoContextoUP',
desempenho = '$desempenhoUP',
encaminhamento = '$encaminhamentosUP',
avanco = '$avancosUP'
where ID = $codigo";
$query = mysql_query($sql) or die("SQL:" . $sql . " - ERRO:" . mysql_error());
}
