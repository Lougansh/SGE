...
$conteudoID = 0;
$titulo = '<div align="center"><h3>CONTEÚDOS - '.$anoTurma.'</h3></div>';
	...
	$recurso =        str_replace( "\n",'</li><li>',$linha['Recurso']);
	$conteudo = $conteudo.'<p><b><font color="blue">Aula '.$conteudoID.'</font>: <font color="gray">'.$linha['Conteudo'];
	}
...
elseif (isset($_POST['btnPesquisar'])){
	...
	while ($linha = mysql_fetch_array($query)) {
		...
		$conteudo = '<p><b><font color="blue">CONTEÚDO</font>: <font color="gray">'.$linha['Conteudo'];
	}
	mysql_free_result($query);
}
